...
Produces a single interactive HTML file with a layer per forecast day,
color-coded by depth: red (no-go), yellow (caution), green (safe),
gray (no data).
"""
...
class MapBuilder:
    ...
    def build_multi_day_map(
        self,
        daily_summaries: List[Dict],
        grid_bounds: list,
    ) -> folium.Map:
        """
        Build a Folium map with one toggleable layer per forecast day.

        Args:
            daily_summaries: From DepthCalculator.compute_daily_summaries
        ...
        # One layer per forecast day
        for i, summary in enumerate(daily_summaries):
            date_str = summary["date"].strftime("%a %m/%d")
            low_time = summary["min_depth_hour"].strftime("%I:%M %p")
            eff_low = summary["tide_at_min"] + summary["setdown_at_min"]

            layer_name = (
                f"{date_str}  |  Low: {eff_low:+.1f} ft @ {low_time}"
            )

            fg = folium.FeatureGroup(
                name=layer_name,
                show=(i == 0),  # Show first day by default
            )
            ...
            # Conditions marker
            self._add_conditions_marker(fg, summary)
    ...
    def _add_conditions_marker(self, fg, summary):
        """Add a popup marker showing tide/wind conditions for that day."""
        eff_low = summary["tide_at_min"] + summary["setdown_at_min"]
        popup_html = (
            f"<div style='font-family:Arial; font-size:13px; min-width:200px;'>"
            f"<b>{summary['date'].strftime('%A %m/%d/%Y')}</b><br><br>"
            f"<b>Worst-case conditions:</b><br>"
            f"Tide: {summary['tide_at_min']:+.1f} ft MLLW<br>"
            f"Wind setdown: {summary['setdown_at_min']:+.1f} ft<br>"
            f"<b>Effective low: {eff_low:+.1f} ft MLLW</b><br>"
            f"at {summary['min_depth_hour'].strftime('%I:%M %p')}<br>"
            f"</div>"
        )
        folium.Marker(
            location=self.center,
    ...
    def _add_legend(self, m: folium.Map):
        ...
        ">
            <b style="font-size:14px;">Predicted Depth (worst case)</b><br><br>
